 ...
 * count of grouped problems ("N problemer") plus the raw sub-part total
 * ("M delopgaver" — `lr_item_render.group_key` collapses multi-part exam
 * questions into one problem; the sub-part count is what the old flat count
 * used to show, kept as context rather than dropped), a start button that
 * opens `ExerciseSession`, and a friendly empty state when this user has
 * flagged every item in the pool.
 *
...
export function InfinitePanel() {
  ...
  return (
    <section className="flex flex-col gap-2 md:gap-3">
      ...
      {counts !== undefined && (counts === null || counts.problems === 0) && !error && (
      ...
      {counts && counts.problems > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-black/[0.06] bg-white p-3 shadow-[0_1px_8px_rgba(0,0,0,0.05)] md:gap-5 md:rounded-2xl md:p-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] uppercase tracking-wide text-[#6E6E78]/80">
              problemer · {counts.parts} delopgaver
            </div>
          </div>
          <button
            type="button"
