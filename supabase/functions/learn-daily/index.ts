...
// ── The READ-GATE (v5) ─────────────────────────────────────────────────────
//
// A fourth thing the graph cannot know: WHETHER THE BOOK HAS BEEN READ. The
// calendar ceiling says where the course is; it says nothing about whether the
// learner has actually read the chapter. v5 gates NEW material on reading
// progress from `lr_materials` + `lr_progress_events` (the Learn tab's
// tracker): a concept is served fresh only once its chapter is READ in the
// course's linked material. Composed with the existing evidence gate on
// review, this yields the intended lifecycle — READ → PRACTICED → RETAINED —
// so nothing unread is ever practiced, and nothing unpracticed ever enters
// spaced repetition. A course with NO linked material keeps v4 behaviour (the
// gate does not invent a requirement the tracker cannot express), and the
// verdict reports `read_chapter` + how many candidates the gate held back, so
// an empty fresh pool is attributable to "read the book first" rather than
// mysterious.
//
// ── What a card is ─────────────────────────────────────────────────────────
//
...
// The Learn tab's reading tracker — the read-gate's input.
interface LrMaterialRow {
  id: string;
...
interface CourseVerdict {
  ...
  read_chapter: number | null;
  quota: number;
  cards: number;
  gate: "strict" | "relaxed";
  urgency: number;
  urgency_reason: string | null;
  pool: { fresh: number; due: number; edges: number; unread: number };
  note?: string;
}
...
// ── Reading position → read chapter ────────────────────────────────────────

/**
 * How far into the BOOK the learner has read, as a chapter number — the
 * read-gate's unit. Two honest mappings, and null everywhere else:
 *   unit_label 'chapters' → the position IS the chapter count (floor).
 *   chapter_pages         → highest chapter whose page range END ≤ position;
 *                           a chapter half-read is not read.
 * Null means "this material cannot express chapters" and the caller must NOT
 * gate on it — absent is not zero, and a page-tracked book without a map must
 * not silently lock a whole course.
 */
function readChapterOf(m: LrMaterialRow, position: number): number | null {
 ...
 *
 * The READ-GATE is different in kind and deliberately hard: `readCh` non-null
 * means the course has a linked, chapter-mapped material, and a concept whose
 * chapter is beyond what has been read is NOT a candidate at all — the fix is
 * to read, and the day's reading block is exactly that. `unread` counts what
 * the gate held back so the verdict can say so.
 */
function rankFresh(
  concepts: Concept[],
  ceiling: number,
...
async function generate(
  ...
  // ── The read-gate's input: the Learn tab's tracked materials ─────────────
  // Any status counts — pausing a book must not un-read its chapters.
  const materialRows = unwrap<LrMaterialRow>(
    "lr_materials",
    await db.from("lr_materials")
      .select("id, course_id, chapter_prefix, unit_label, start_unit, chapter_pages")
      .eq("user_id", userId).not("course_id", "is", null),
  );
  const positions = new Map<string, number>();
  if (materialRows.length) {
    const posRows = unwrap<PositionRow>(
      "lr_progress_events",
      await db.from("lr_progress_events")
        .select("material_id, units_to")
        .eq("user_id", userId)
        .in("material_id", materialRows.map((m) => m.id))
        .not("units_to", "is", null),
    );
    for (const p of posRows) {
      if (p.units_to == null) continue;
      const cur = positions.get(p.material_id) ?? 0;
      if (p.units_to > cur) positions.set(p.material_id, p.units_to);
    }
  }
  // (course, prefix) → chapters read. Several materials mapping to the same
  // book take the furthest one.
  const readChBy = new Map<string, number>();
  for (const m of materialRows) {
    if (m.course_id == null || !m.chapter_prefix) continue;
    const pos = Math.max(m.start_unit ?? 0, positions.get(m.id) ?? 0);
    const ch = readChapterOf(m, pos);
    if (ch == null) continue;
    const key = `${m.course_id}:${m.chapter_prefix.toUpperCase()}`;
    if (ch > (readChBy.get(key) ?? -1)) readChBy.set(key, ch);
  }
  ...
  const perCourse = enrollments.map((e) => {
    ...
    // otherwise "why does X need Y?" is two unknowns and a guess. Under the
    // read-gate BOTH ends must also be read: a link card about an unread
    // chapter references content the learner has never seen.
    ...
    return {
      e, phase, ceiling, week, gate, urgency, urgencyReason, readCh,
      pool: { fresh, due, edges, gate, unread } as CoursePool,
    };
  });
  ...
  perCourse.forEach((p, i) => {
    ...
    verdicts.push({
      ...
      pool: {
        ...
        unread: p.pool.unread,
      },
      ...(weights[i] === 0
        ? {
          note: p.pool.unread > 0
            ? "alle nye koncepter venter på læsning — dagens læseblok låser dem op"
            : "ingen brugbare koncepter i vinduet — mangler grafen kanter?",
        }
        : {}),
    });
  });
